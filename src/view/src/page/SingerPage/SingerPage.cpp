...
BOOL CSingerPage::RefreshTree(CMediaSet *pSingerObject)
{
	...
	if (NULL == pSingerObject || E_MediaSetType::MST_Singer != pSingerObject->m_eType)
	{
		if (0 != m_uSingerID)
		{
			auto pSinger = SingerMgr.GetSubSet(E_MediaSetType::MST_Singer, m_uSingerID);
			if (NULL != pSinger)
			{
...
void CSingerPage::OnNMRclickTree(NMHDR *pNMHDR, LRESULT *pResult)
{
	*pResult = 0;
	...
	m_MenuGuard.EnableItem(ID_OtherLanguage, NULL != pSingerObject);
	m_MenuGuard.EnableItem(ID_TlLanguage, NULL != pSingerObject);
	m_MenuGuard.EnableItem(ID_RsLanguage, NULL != pSingerObject);
	m_MenuGuard.EnableItem(ID_FrLanguage, NULL != pSingerObject);
